...
| `chain-id` | `BASE` |
| Spec | [`specs/base.json`](https://github.com/Magma-Devs/smart-router/blob/main/specs/base.json) |

## Method surface

The Base spec **imports `ETH1`**, so it serves the standard Ethereum JSON-RPC method surface (`eth_*`, `net_*`, `web3_*`, plus `debug_*`/`trace_*` on capable upstreams). See [Ethereum → Supported method families](ethereum.md#supported-method-families) and [Upstream capabilities](ethereum.md#upstream-capabilities) — the same add-ons (`debug`, `trace`, `archive`) apply.
